...
function camera.firstPersonMovement(dt)
    ...
    if moveX ~= 0 or moveY ~= 0 then
		...
		sign = (sign > 0 and 1) or (sign < 0 and -1) or 0
		local cosPitch = sign*math.max(math.abs(math.cos(fpsController.pitch)), 0.00001)
        camera.position[1] = camera.position[1] + math.sin(fpsController.direction + angle) * speed * dt*cosPitch
        camera.position[3] = camera.position[3] + math.cos(fpsController.direction + angle) * speed * dt*cosPitch
        cameraMoved = true
    end
